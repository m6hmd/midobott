        ...
        for username in mes:
            ...
            if "Public channel" in response.text or "Open channel" in response.text:
                if 'data-url="' in response.text:
                    ...
                    bot.send_message(message.chat.id, f"URL :: {url}\nTime :: {responseTime.replace('января в','1').replace('февраля','2').replace('марта в','3').replace('мая','5').replace('июля','7').replace('августа','8').replace('октября','10') .replace('июня','6').replace('апреля','4').replace('сентября','9').replace('ноября','11').replace('декабря','12').replace(' ', '-')}")
                else:
